...
export function PurgePiston(): JSX.Element {
  ...
  const { rive, RiveComponent } = useRive({
    src: 'assets/water.riv',
    stateMachines: 'State Machine',
    autoplay: true
  });

  const animateToPosition = useCallback(
    (targetPosition: number, cp: number) => {
      const startPosition = cp;
      const startTime = performance.now();
      ...
      const animate = () => {
        ...
        const clampedPosition = Math.max(0, Math.min(newPosition, TOTAL_LEVEL));

        if (clampedPosition > TOTAL_LEVEL) {
          return;
        }
        ...
        if (rive) {
          ...
          rive
            ...
            .find((e) => e.name === 'Level').value = clampedPosition;
        }
  ...
  useEffect(() => {
    if (rive !== null) {
      const cp = rive
        .stateMachineInputs('State Machine')
        .find((e) => e.name === 'Level').value as number;
